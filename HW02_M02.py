...
warn_count  = re.compile(r"\bWARN\b*\b[[a-zA-Z]+\]")
...
def check_password(pw:str):
    is_strong = (
        len(pw) >= 10
        and lowercase.search(pw) is not None
        and uppercase.search(pw) is not None
        and digit_case.search(pw) is not None
        and symbol_case.search(pw) is not None 
        and space_case.search(pw) is None 
    )
    return "STRONG" if is_strong else "WEAK"

### step 3: Use case. Test list test_pw
for i in test_pw:
    print(i, "password is", check_password(i))
